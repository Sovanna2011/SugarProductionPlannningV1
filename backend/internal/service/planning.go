...
// MatrixSetRequest saves several grids in one call: several products, each
// over the same span of days, written all-or-nothing.
//
// A planner works on a week of the whole plant at once — raw sugar, molasses
// and bagasse across every mill line — so making them save one product at a
// time would be four round trips that can half-fail. One transaction, one
// version check, one audit entry.
type MatrixSetRequest struct {
	CompanyID     int64
	SeasonID      int64
	VersionID     int64
	DateFrom      time.Time
...
// MatrixSeriesInput is one product's grid within the set.
type MatrixSeriesInput struct {
	MovementTypeID int64
	MaterialID     int64
	ProcessID      *int64
	WarehouseID    *int64
...
// SaveMatrix upserts the Date × Line grid of one product. The operation is
// idempotent: the same payload can be replayed without creating duplicate
// rows, because the upsert is keyed on the item business key.
func (s *PlanningService) SaveMatrix(ctx context.Context, req MatrixRequest, actorID int64) (*MatrixResponse, error) {
	saved, err := s.SaveMatrixSet(ctx, MatrixSetRequest{
...
// SaveMatrixSet writes several products' grids over several days in one
// transaction. Either the whole plan lands or none of it does — a planner
// editing a week of the plant should never be left with three products saved
// and the fourth rejected.
func (s *PlanningService) SaveMatrixSet(ctx context.Context, req MatrixSetRequest, actorID int64) ([]MatrixResponse, error) {
	if req.DateTo.Before(req.DateFrom) {
		return nil, apperrors.ErrInvalidDateRange
	}
	if len(req.Series) == 0 {
		return nil, apperrors.ErrValidation.
			Msgf("a matrix save must carry at least one product").
			WithDetails(apperrors.Detail{Field: "series"})
	}
	...
	err := s.uow.Do(ctx, func(ctx context.Context) error {
		...
		responses = make([]MatrixResponse, 0, len(req.Series))
		for _, series := range req.Series {
			saved, err := s.saveSeries(ctx, req, series, version, season)
			if err != nil {
				return err
			}
		...
		s.auditSvc.Record(ctx, audit.Event{
			TableName: "plan_items", RecordID: &req.VersionID, CompanyID: &req.CompanyID,
			Action: model.AuditUpdate,
			NewValues: map[string]any{
...
// saveSeries writes one product's grid. It runs inside the caller's
// transaction and assumes the version has already been checked.
func (s *PlanningService) saveSeries(ctx context.Context, set MatrixSetRequest,
	series MatrixSeriesInput, version *model.PlanningVersion, season *model.Season) (*MatrixResponse, error) {

	if _, _, err := s.refs.MaterialForCompany(ctx, set.CompanyID, series.MaterialID); err != nil {
		return nil, err
	}

	header, err := s.resolveMatrixHeader(ctx, set, series, version, season)
	if err != nil {
		return nil, err
	}

	items := make([]model.PlanItem, 0, len(series.Cells))
	provided := make(map[string]bool, len(series.Cells))

	for _, cell := range series.Cells {
		if !season.Contains(cell.PlanDate) {
			return nil, apperrors.ErrDateOutsideSeason.WithDetails(apperrors.Detail{
				Field: "planDate", Value: cell.PlanDate.Format("2006-01-02"),
			})
		}
		if err := s.refs.ProductionLine(ctx, set.CompanyID, cell.ProductionLineID); err != nil {
			return nil, err
		}
		...
		if cell.Quantity.IsNegative() {
			return nil, apperrors.ErrValidation.Msgf("a planned quantity must not be negative").
	...
	// A full save treats an omitted cell as a deletion; a partial save leaves
	// everything it did not mention alone. Deletion is scoped to this product,
	// so saving raw sugar never clears the molasses grid beside it.
	if !set.PartialUpdate {
		existing, err := s.plans.MatrixCells(ctx, set.CompanyID, set.VersionID,
			series.MovementTypeID, series.MaterialID, series.ProcessID, set.DateFrom, set.DateTo)
		if err != nil {
			return nil, err
		}
		var obsolete []int64
	...
	return s.loadMatrix(ctx, set.CompanyID, version, series.MovementTypeID,
		series.MaterialID, series.ProcessID, set.DateFrom, set.DateTo)
}
...
// GetMatrixSet returns every grid the version plans in the window — each
// product the planner has touched, over the same days. It is what the planning
// screen opens on, so a planner sees the whole plant instead of picking one
// product before seeing anything.
func (s *PlanningService) GetMatrixSet(ctx context.Context, companyID, versionID int64,
	...
	series, err := s.plans.MatrixSeries(ctx, companyID, versionID, from, to)
	if err != nil {
		return nil, err
	}

	out := make([]MatrixResponse, 0, len(series))
	for _, key := range series {
		matrix, err := s.loadMatrix(ctx, companyID, version, key.MovementTypeID,
			key.MaterialID, key.ProcessID, from, to)
		if err != nil {
			return nil, err
		}
		matrix.MaterialCode = key.MaterialCode
		matrix.MaterialName = key.MaterialName
		matrix.ProcessCode = key.ProcessCode
		matrix.UOMID = key.UOMID
		matrix.UOMCode = key.UOMCode
...
// the line lives on each item instead. Products sharing a movement type share
// the document — the material lives on the item.
func (s *PlanningService) resolveMatrixHeader(ctx context.Context, set MatrixSetRequest,
	series MatrixSeriesInput, version *model.PlanningVersion, season *model.Season) (*model.PlanHeader, error) {

	header, err := s.plans.FindHeaderByKey(ctx, set.CompanyID, set.VersionID, series.MovementTypeID, nil)
	if err != nil {
		return nil, err
	}
	...
	if header != nil {
		...
		changed := false
		if set.DateFrom.Before(header.DateFrom) {
			header.DateFrom = set.DateFrom
			changed = true
		}
		if set.DateTo.After(header.DateTo) {
			header.DateTo = set.DateTo
			changed = true
		}
		if changed {
			if err := s.plans.UpdateHeader(ctx, set.CompanyID, header); err != nil {
				return nil, err
			}
		}
		return header, nil
	...
	documentNo, err := s.numbering.Next(ctx, set.CompanyID, model.NumberObjectPlan, set.DateFrom)
	if err != nil {
		return nil, err
	}
